...
echo ==========================================
echo.
echo Spustam web na porte 8000...
echo.

if not exist "%~dp0artisan" (
    echo.
    echo CHYBA: Súbor artisan nebol nájdený.
...
if errorlevel 1 (
    echo CHYBA: Nie je dostupne PHP ani Python.
    echo Nainstalujte PHP 8.2+ alebo Python, pripadne pouzite Docker.
    echo.
    pause
    exit /b 1
)

echo UPOZORNENIE: Bezi iba staticky fallback server (iny vizual ako Laravel).
start "" http://localhost:8000
